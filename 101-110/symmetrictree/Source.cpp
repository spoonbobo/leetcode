...
// beats 100% time, but very space expensive. Can be optimized.

// O(n) symmetric check
bool symmetric(vector<TreeNode* >& left_tn, vector<TreeNode* >& right_tn) {
    for (int i = 0, j = right_tn.size() - 1; i < left_tn.size() && j >= 0; i++, j--) {
...
bool allnull(vector<TreeNode* >& left_tn, vector<TreeNode* >& right_tn) {
    for (int i = 0, j = right_tn.size() - 1; i < left_tn.size() && j >= 0; i++, j--) {
        if (left_tn[i]->val != -150 || right_tn[j]->val != -150) return false;
    }
    return true;
}

bool check(vector<TreeNode* >left_tn, vector<TreeNode* >right_tn) {
    if (allnull(left_tn, right_tn))
        return true;
    if (!symmetric(left_tn, right_tn))
        return false;

    vector<TreeNode* > new_left;
    for (int i = 0; i < left_tn.size(); i++) {
    ...
    return check(new_left, new_right);
}
